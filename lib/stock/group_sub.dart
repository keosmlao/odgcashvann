...
class GroupSub extends StatefulWidget {
  final String groupMain; // Use 'final' for StatefulWidget properties
  const GroupSub({super.key, required this.groupMain});

  @override
  State<GroupSub> createState() => _GroupSubState();
}
...
class _GroupSubState extends State<GroupSub> {
  ...
  Future<void> _showData() async {
    ...
    try {
      var response = await get(
        Uri.parse(
          "${MyConstant().domain}/vansale_groupsub/${widget.groupMain}",
        ),
      );
      ...
      if (response.statusCode == 200) {
        ...
        _showInfoSnackBar(
          'Failed to load sub groups: ${response.statusCode}',
          Colors.red,
        );
      ...
      _showInfoSnackBar('Error loading sub groups: $error', Colors.red);
      print("Error: $error");
    } finally {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text(
          "ກຸ່ມສິນຄ້າຍ່ອຍ 1", // Changed title to reflect "Sub Group 1"
          style: TextStyle(
            color: Colors.white,
            fontSize: 20,
          ...
          ? Center(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    "ບໍ່ພົບກຸ່ມສິນຄ້າຍ່ອຍ 1.", // Message for no sub groups
                    style: TextStyle(
                  ...
                  Text(
                    "ກະລຸນາກວດສອບກຸ່ມຫຼັກທີ່ເລືອກ.", // Suggestion
                    style: TextStyle(
                      fontFamily: 'NotoSansLao',
                      fontSize: 15,

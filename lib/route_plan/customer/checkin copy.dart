...
class _CheckInState extends State<CheckIn> {
  ...
  bool _isLocationLoading = true; // Keep this true until location is fetched
  ...
  void initState() {
    super.initState();
    _getCurrentTime();
    _getLocation(); // Start fetching location immediately
  }
  ...
  Future<void> _getLocation() async {
    ...
    try {
      ...
      setState(() {
        ...
        _isLocationLoading = false; // Location fetched, stop loading
        if (_currentLocation != null) {
          ...
          // This ensures the map updates even if it was created before location was ready
            ...
            child: _isLocationLoading
                ...
                : GoogleMap(
                    onMapCreated: (controller) {
                      ...
                      // When the map is created, if location is already available,
                    ...
                    // Use a default initial position (0,0) if location isn't ready.
                    // The camera will be animated to the correct location once _getLocation completes.
                    ...
                    markers:
                        _markers, // This will be updated by setState in _getLocation
                  ),
          ),
  ...
  Widget _buildCheckedInView() {
    final DateFormat checkinFullDisplayFormat = DateFormat(
      'yyyy-MM-dd HH:mm:ss',
    );
    ...
    try {
      if (widget.checkin.isNotEmpty) {
        final DateTime parsedCheckin = checkinFullDisplayFormat.parse(
          widget.checkin,
        );
        displayCheckinTime = checkinFullDisplayFormat.format(parsedCheckin);
      }
    } catch (e) {
    ...
    print("Checked-in location: $checkedInLocation");
          ...
          Container(
            ...
            child: GoogleMap(
              onMapCreated: (controller) {
                _mapController = controller;
                print(
                  "Map created: ${checkedInLocation.latitude}, ${checkedInLocation.longitude}",
                );
                // Ensure the camera moves to the checked-in location when map is created
                _mapController!.animateCamera(
                  CameraUpdate.newLatLngZoom(checkedInLocation, 16),
                );
              },
              initialCameraPosition: CameraPosition(
                target:
                    checkedInLocation.latitude != 0 ||
                        checkedInLocation.longitude != 0
                    ? checkedInLocation
                    : const LatLng(
                        17.982175233109366,
                        102.61610563248911,
